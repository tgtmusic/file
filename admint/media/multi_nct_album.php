...
if (!$_POST['submit']) {
	...
	$album_info = str_replace("<br>","",str_replace("<br/>",". ",str_replace("<br />",". ",$album_info)));
...
<div class="input-group">
    	...
    	<select class="form-control" name="singer_type_a" onChange="new_stype(this.value)">
			...
			<option value=2>Album Âu Mỹ</option>
            <option value=3>Album Châu Á</option>
		</select>
		</div>
                  </div><br/>
...
for ($i=1;$i<=$total_links;$i++) {
    ...
    $song = explode('titleplay="', $url_song[$i]);
    $song = explode(' - ', $song[1]);
    $song = loaibo($song[0]);
...
<div class="input-group ">
        	...
        	<option value=2>Ca sĩ Âu Mỹ</option>
       		<option value=3>Ca sĩ Châu Á</option>
		</select>
		 </div>
				  </div><br/>
...
<?php
	...
	$t_title_ascii = strtolower(get_ascii($t_title_ascii));
		...
		$song_id = them_moi_media($t_singer, $t_cat, $t_url,'1',$t_title,$t_title_ascii,'0','0',$_SESSION['admin_id'],$img,$t_lyric);
		}
	$list .=  $song_id;
	}
