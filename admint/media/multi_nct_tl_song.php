 ...
 <div class="box box-info">
                <div class="box-header with-border">
                  <h3 class="box-title">Mutil Add NTC THỂ LOẠI SONG</h3>
...
+ Link post : http://www.nhaccuatui.com/bai-hat/nhac-tre-moi.html</p>
...
for ($i=1;$i<=$total_links;$i++) {
	$play_url = explode('<a href="http://www.nhaccuatui.com/', $url_song[$i]);
	$play_url = explode('"', $play_url[1]);
    $txt2='http://www.nhaccuatui.com/'.$play_url[0];
    $song = explode('title="Thêm bài hát ', $url_song[$i]);
    $song = explode(' - ', $song[1]);
    $song = $song[0];
	$singer = explode('title="Thêm bài hát '.$song.' - ', $url_song[$i]);
    $singer = explode(' vào playlist yêu thích', $singer[1]);
    $singer = $singer[0];
...
<?php
	...
	$t_title_ascii = strtolower(get_ascii($t_title_ascii));
		...
		mysqli_query($link_music,"INSERT INTO table_data (m_singer,m_cat,m_url,m_type,m_title,m_title_ascii,m_is_local,m_poster,m_lyric,m_time) VALUES ('".$t_singer."','".$t_cat."','".$t_url."','1','".$t_title."','".$t_title_ascii."','0','".$_SESSION['admin_id']."','".$t_lyric."','".NOW."')");
		}}
	}
	echo "ADD MEDIA Thành công !<meta http-equiv='refresh' content='0;url=index.php?act=media&mode=multi-nct-tl-song'>";
}
}
?>
