...
+ Link post : http://nhac.vn/nghe-si/viet-nam</p>
...
if (!$_POST['submit']) {
	...
	$url_song = explode('thumb-hover', $url);
...
for ($i=1;$i<=$total_links;$i++) {
    ...
    $url_news=$play_url[0].'/tieu-su';
	$url2 = file_get_contents($url_news);
	$s_name = explode('<title>Tiểu sử ', $url2);
    $s_name = explode('</title>', $s_name[1]);
    $s_name = $s_name[0];
	$info = explode('Thông tin thêm:</strong>', $url2);
    $info = explode('</div>', $info[1]);
    $info = $info[0];
    $img = explode('<img class="aavatar" src="', $url2);
    $img = explode('"', $img[1]);
	#$img ='http://avatar.nct.nixcdn.com/singer/avatar/'.$img[0];
	  $img = $img[0];
	$big_img = explode('<img class="artist-cover" src=', $url2);
    $big_img = explode(' ', $big_img[1]);
    #$big_img = $big_img[0];
	 $big_img = 'theme/images/awall.png';
	#$big_img = str_replace('/web/images/awall.png',"theme/images/awall.png",$big_img[0]);
	$tenthat = explode('Tên thật:</strong> ', $url2);
    $tenthat = explode('</p>', $tenthat[1]);
    $tenthat = $tenthat[0];
	$ngaysinh = explode('Ngày sinh:</strong> ', $url2);
    $ngaysinh = explode('</p>', $ngaysinh[1]);
    $ngaysinh = $ngaysinh[0];
	$quocgia = explode('Quốc gia:</strong> ', $url2);
    $quocgia = explode('</p>', $quocgia[1]);
    $quocgia = $quocgia[0];
	$quequan = explode('Quê quán:</strong> ', $url2);
...
for ($i=0;$i<=$total_links;$i++) {
		...
		$new_singer = htmlchars(stripslashes($_POST['singer_name'][$i]));
	...
	if ($new_singer) {
			...
			$singer 			= them_moi_s($new_singer,$t_cat,$singer_img,$singer_big_img,$singer_info,$singer_type,$singer_tenthat,$singer_ngaysinh,$singer_quocgia,$singer_quequan);
			}
			}
	//mss (" add Ok ! ","list_singer.php");
	echo "Thành Công","<meta http-equiv='refresh' content='0;url=index.php?act=singer&mode=multi-singer-nhacvn'>";
