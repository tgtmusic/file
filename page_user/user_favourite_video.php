<?php 
$tong_bh = "SELECT m_id  FROM table_data WHERE  m_poster = '".$user_id."' AND m_type = 2 AND m_mempost = 1 ORDER BY m_id DESC LIMIT ".LIMITSONG;
$tong_so_bai_hat=mysqli_query($link_music,"$tong_bh") or die(mysqli_error());
...
$link_s = 'u/'.$user_name.'/video';
	// phan trang
$tong_bh = "SELECT m_id  FROM table_data WHERE  m_poster = '".$user_id."' AND m_type = 2 ORDER BY m_id DESC LIMIT ".LIMITSONG;
$tong_so_bai_hat=mysqli_query($link_music,"$tong_bh") or die(mysqli_error());
	$bai_hat = mysqli_num_rows($tong_so_bai_hat);
	$sql_tt = "SELECT m_id  FROM table_data WHERE m_id IN ($list) AND m_type = 2 ORDER BY m_id DESC LIMIT ".LIMITSONG;
	$phantrang = linkPage($sql_tt,HOME_PER_PAGE,$page,$link_s."&p=#page#","");
	$rStar = HOME_PER_PAGE * ($page -1 );
	$arr_song = $mlivedb->query(" m_id, m_title, m_singer, m_viewed, m_img, m_type, m_cat, m_poster, m_time ","data"," m_id IN ($list) AND m_type = 2 ORDER BY m_id DESC LIMIT ".$rStar .",". HOME_PER_PAGE,"");
?>  
