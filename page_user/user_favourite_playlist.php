...
<?php $tong_bh = "SELECT album_id  FROM table_album WHERE  album_poster = '".$user_id."' AND album_type = 1 ORDER BY album_id DESC LIMIT ".LIMITSONG;	
...
$link_s = 'u/'.$user_name.'/playlist';
	
	$sql_tt = "SELECT album_id  FROM table_album WHERE album_id IN ($list) AND album_type = 0 ORDER BY album_id DESC LIMIT ".LIMITSONG;
	$phantrang = linkPage($sql_tt,HOME_PER_PAGE,$page,$link_s."&p=#page#","");
	$rStar = HOME_PER_PAGE * ($page -1 );
	$arr_album = $mlivedb->query("album_id, album_name, album_singer, album_viewed, album_img, album_type, album_cat, album_poster, album_time, album_song, album_info","album"," album_id IN ($list) AND album_type = 0 ORDER BY album_id DESC LIMIT ".$rStar .",". HOME_PER_PAGE,"");
			?> 
