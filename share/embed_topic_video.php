...
if(isset($_GET["list"])) $list = $myFilter->process($_GET['list']);
$list	=	del_id($id);
if(isset($_GET["type"])) $type = $myFilter->process($_GET['type']);

if($type == 'topic') { $where = "find_in_set(".$list.",m_topic) AND m_type = 2"; $sql_order = " ORDER BY m_id DESC"; }
...
$album_list_song = "[";	
	$arr = $mlivedb->query(" m_id, m_url, m_title, m_singer, m_is_local, m_lrc, m_img,m_lyric ","data"," $where $sql_order");
