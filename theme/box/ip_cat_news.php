...
for($z=0;$z<count($cat);$z++) {
	$cat_url = url_link($cat[$z][1],$cat[$z][0],'news-cat');
?>
<div class="subcol menu-col-1">
<div class="title-menu"><a href="<?=$cat_url;?>"><?=$cat[$z][1];?></a></div>
	        <?php
        $cats1 = $mlivedb->query("cat_news_id, cat_news_name","catnews"," cat_news_sub_id = '".$cat[$z][0]."' ORDER BY cat_news_order ASC LIMIT 7");
        for($i=0;$i<count($cats1);$i++) {
            $cats1_url = url_link($cats1[$i][1],$cats1[$i][0],'news-cat');
        ?>
